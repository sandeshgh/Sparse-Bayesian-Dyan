...
class H36Mseq(Dataset):
    def __init__(self, len_in, len_out, data_dir):
        ...
        self.seq_length_in = len_in
        self.seq_length_out = len_out

        self.read_all_data(self.actions, data_dir) 
    ...
    def get_train_batch(self, batch_size):
        ...
        for i in xrange( batch_size ):
            ...
            # Add the data
            encoder_inputs += [np.expand_dims(data_sel[0:self.seq_length_in, :], 0)]
            decoder_outputs += [np.expand_dims(data_sel, 0)]
        return torch.from_numpy(np.concatenate(decoder_outputs, axis=0)).type(torch.FloatTensor)
    ...
    def get_test_batch(self, action):
        ...
        #print('get_test,seed',seeds)
        for i in xrange( batch_size ):

            _, subsequence, idx = seeds[i]
            idx = idx + 50
            #print('get_test_batch, sequence', subject,action, subsequence, idx)

            data_sel = self.test_set[ (subject, action, subsequence, 'even') ]
            data_sel = data_sel[(idx-source_seq_len):(idx+target_seq_len) ,:]

            encoder_inputs += [np.expand_dims(data_sel[0:source_seq_len, :], axis=0)]
            decoder_outputs += [np.expand_dims(data_sel, axis=0)]
            currentAction = self.label_cvt.transform([action])

            yhat[i, currentAction[0]] = 1

        inputs = torch.from_numpy(np.concatenate(encoder_inputs, axis=0)).type(torch.FloatTensor)
        gts = np.concatenate(decoder_outputs, axis=0)

        return inputs, gts, yhat
    ...
    def get_srnn_gts(self, one_hot, to_euler=True):
        ...
        for action in self.actions:
            print('get_gts action', action)
            srnn_gt_euler = []
            _,srnn_expmap, _  = self.get_test_batch(action)
            srnn_expmap = np.squeeze(srnn_expmap) # 75x54
            # expmap -> rotmat -> euler
                ...
                if to_euler:
                    for j in np.arange(denormed.shape[0]):
                        for k in np.arange(0, 97, 3):
                            idx = [k, k+1, k+2]
                            denormed[j, idx] = data_utils.rotmat2euler(
                                data_utils.expmap2rotmat(denormed[j, idx]))
    ...
    def compute_test_error(self,action, pred_pose,srnn_gts_euler,one_hot):
        # compute_test_error(self,action, pred_pose,srnn_gts_expmap,srnn_gts_euler,one_hot,samples_fname):
        """
        Compute the test error 
        """

        # pred_pose = (np.squeeze(pred_pose))[:, 50:, ]  # 8x25x54
        ...
        predict_len = 25
        mean_errors = np.zeros((len(predict_expmap), predict_len))
        for i in np.arange(8):
            
            eulerchannels_pred = np.copy(predict_expmap[i])  # 25 x 99
            for j in np.arange(eulerchannels_pred.shape[0]): # j = 0 : 25
                for k in np.arange(0, 97, 3):
                    idx = [k, k+1, k+2]
            ...
            srnn_gts_euler[i][:, 0:6] = 0 ## Fixed by ZHEN, we also need
            ...
            idx_to_use = np.where(np.std(srnn_gts_euler[i], 0) >= 1e-4)[0] #check >=1e-4
            # print(idx_to_use)
            # idx_to_use = self.dim_to_use
            euc_error = np.power(srnn_gts_euler[i][50:, idx_to_use] - eulerchannels_pred[50:, idx_to_use], 2)
